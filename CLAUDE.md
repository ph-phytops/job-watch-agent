...
```bash
# --profile is mandatory: it names whose search this is. The owner is "pierre".
uv run jobwatch.py --profile pierre            # collect -> filter -> score -> digest -> notify
...
- `run_daily.cmd`: Windows Task Scheduler; `uv run --locked`, appending to
  `profiles/pierre/data/run.log`. uv must be on the PATH of the account
  running the task.
...
5. **Review (optional, `--llm`)**: a local-only qualitative pass. The
   ...
   **`--llm` never runs in CI and never writes shared state**: no digest, no
   ...
   The tracked template is `profiles.example/someone/data/profile.md`.
   Any failure is non-fatal: the run
...
`profiles/*/data/*` gitignore rule and is committed, because the GitHub Actions run needs
...
and `run_daily.cmd` pass.

`profiles/` is gitignored, `profiles.example/someone/` is the tracked template.
Another person's search criteria, memory and credentials must never reach this
public repository.

**A profile loads only its own `.env`, with no fallback to the root one.** The
fallback looks harmless and is not: a profile missing its credentials would
silently open the owner's mailbox and mail them a digest built from someone
else's criteria. Missing credentials are reported and the run continues on the
profile's ATS targets.

Two traps when writing a profile config:

- `matches()` does substring matching on lowercased titles, so a short keyword
  catches every word containing it (`"rse"` matches `course`, `diverse`,
  `bourse`). Always use the long form.
- TOML scoping: a person with no ATS target needs `companies = []` placed
...
- `.env` (gitignored, template `profiles.example/someone/.env.example`)
  `JOBWATCH_EMAIL_USER`,
